...
from .io.input_model import _read_input_model
...
def _initialize_hdf5_model(
    config: HDF5ModelConfig,
) -> Model1D:
    source = _read_input_model(config.path)
    ...
    return Model1D(
        ...
        rho=rho,
        domains=find_domains(r),
        additional_variables=source.additional_variables,
    )
...
def initialize_model_1d(
    ...
    if isinstance(config, HDF5ModelConfig):
        return _initialize_hdf5_model(config)

    raise TypeError(
        "config must be a PolytropeConfig, "
        "CompositePolytropeConfig, LegacyModelConfig "
        "or HDF5ModelConfig."
    )
